	...
	if(ac === len){
		...
		$('#sig').replaceWith('<a href="#" class="ui-btn ui-icon-arrow-r ui-btn-icon-top" id="finalizar">Finalizar</a>');
			...
			$("#vi").html('<button class="ui-btn">Preg. # 1'+' de ' + len + '</button>');
	 		$(".preguntaP").html(results.rows.item(0).Pregunta);
			$(".r1").html('1) ' + results.rows.item(0).R1);
			$(".r2").html('2) ' + results.rows.item(0).R2);
			$(".r3").html('3) ' + results.rows.item(0).R3);
			$(".r4").html('4) ' + results.rows.item(0).R4);
			}else{
